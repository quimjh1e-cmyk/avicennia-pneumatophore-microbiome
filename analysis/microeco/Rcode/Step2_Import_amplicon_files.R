...
tmp_microtable <- qiime2meco(
	...
	sample_table = file.path(tmp, "Sample_info.xlsx")
	)

# read sample information into R
tmp_sample <- as.data.frame(read_excel("Input/1.Amplicon/QIIME2_qza/Sample_info.xlsx", col_names = TRUE), stringsAsFactors = FALSE)
...
amplicon_16S_microtable_rarefy$sample_table <- tmp_sample
...
##################################################################
##################################################################
# Other examples below are from the protocol tutorial using example data (QIIME2_converted).
# They are NOT part of this analysis and are commented out to avoid conflicts with our sample names.
...
# # without phylogenetic tree or representative fasta
...
# tmp_sample2 <- as.data.frame(read_excel("Input/1.Amplicon/Sample_info.xlsx", col_names = TRUE), stringsAsFactors = FALSE)
# rownames(tmp_sample2) <- tmp_sample2[, 1]
# tmp_microtable3 <- qiime2meco(
# 	feature_table = file.path(tmp, "feature_table_filter.qza"),
# 	sample_table = tmp_sample2,
# 	taxonomy_table = file.path(tmp, "taxonomy.qza")
# 	)
...
##################################################################
##################################################################
# Example using QIIME2 qza-converted files (tutorial example data only — different sample names)
