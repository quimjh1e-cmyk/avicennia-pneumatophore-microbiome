...
# Rarefaction
# the minimum of sample sequences is over 21000
tmp_microtable$sample_sums() %>% range
amplicon_16S_microtable_rarefy <- tmp$norm(method = "rarefy", sample.size = 21000)
...
# use condition = "season" as the example; it is suggested to be provided according to the analysis context
amplicon_16S_microtable_Wrench <- tmp$norm(method = "Wrench", condition = "season")
